...
# 6. Check for stock panel-launchers conflict
ENABLED=$(dconf read /org/cinnamon/enabled-applets 2>/dev/null || echo "")
if echo "$ENABLED" | grep -q "$STOCK_UUID"; then
    echo ""
    echo "NOTE: Stock panel-launchers ($STOCK_UUID) is currently enabled."
    echo "  Both applets have the 'panellauncher' role."
    echo "  You should remove the stock one after adding this applet:"
    echo "    Right-click panel -> Applets -> find 'Panel launchers' -> Remove"
fi
...
if echo "$ENABLED" | grep -q "$UUID"; then
    ...
    echo "    2. Search for 'Multi-Row Panel Launchers'"
